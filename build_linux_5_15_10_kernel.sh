...
if [ -f "build/linux-5.15.10/.config" ]; then
  # Check the value of CONFIG_DEBUG_INFO
  if grep -q "^CONFIG_DEBUG_INFO=y$" build/linux-5.15.10/.config; then
    # Compile make command based on the number of available processors
    make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j$(nproc) -C linux-5.15.10 O=../build/linux-5.15.10
